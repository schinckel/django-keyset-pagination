...
class KeysetPaginator(Paginator):
    ...
    def __init__(self, object_list, per_page, orphans=0, allow_empty_first_page=True):
        ...
        if not self.keys:
            raise ValueError(
                "Unable to paginate when no keys are provided: "
                "please order the queryset by at least one key."
            )
    ...
    def _get_ordering(self, number):
        ...
        if number[0]:
            return ["-" + key if key[0] != "-" else key.lstrip("-") for key in self.keys]
        return self.keys
...
class KeysetPage(Page):
    ...
    def __repr__(self):
        ...
        return f"<KeysetPage: {self.page_index} of {self.paginator.num_pages}>"
